...
bool ProjectivePointG1::is_zero(ProjectivePointG1 self){
    ...
    bool is_zero = true;
    for(int i = 0; i<fq::Limbs; i++){
        is_zero = is_zero && (static_cast<uint64_t*>(z_)[i] == 0);
    }
    return is_zero;
}
...
AffinePointG1 to_affine(ProjectivePointG1 input){
    if (ProjectivePointG1::is_zero(input)){
        ...
        SyncedMemory px(input.x.size());
        SyncedMemory py(input.x.size());
        SyncedMemory pz(input.x.size());

        void* px_ = px.mutable_cpu_data();
        void* py_ = py.mutable_cpu_data();
        void* pz_ = pz.mutable_cpu_data();

        void* in_x_ = input.x.mutable_cpu_data();
        void* in_y_ = input.x.mutable_cpu_data();
        void* in_z_ = input.x.mutable_cpu_data();

        memcpy(px_, in_x_, px.size());
        memcpy(py_, in_y_, py.size());
        memcpy(pz_, in_z_, pz.size());

        SyncedMemory& one = fq::one();

        //div_mod work on cpu
        SyncedMemory& zinv = div_mod(one, pz);
        SyncedMemory& zinv_squared = mul_mod(zinv, zinv);

        SyncedMemory& x = mul_mod(px, zinv_squared);
        SyncedMemory& mid1 = mul_mod(zinv_squared, zinv);
        SyncedMemory& y = mul_mod(py, mid1);
...
ProjectivePointG1 add_assign_mixed(ProjectivePointG1 self, AffinePointG1 other){
    if (AffinePointG1::is_zero(other)){
        SyncedMemory x(self.x.size());
        SyncedMemory y(self.y.size());
        SyncedMemory z(self.z.size());
        ...
        SyncedMemory x(other.x.size());
        SyncedMemory y(other.y.size());
